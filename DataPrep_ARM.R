...
#stores <- read.csv("/home/kodyandrew/LoCardData/data_hiroshi/LoCardData_stores.csv", header=T)
### >>>load stores ---------- 
stores <- read.csv("data/LoCardData_stores.csv", header=T)
### No long and lag, this is generated by LoCardData_sotre_clean.R
### >>>load distainces ---------
...
if(bool_stats_DEMOG){
  ...
  storeSES <- dbGetQuery(geoH, 
    "select distinct store_postal_codes,
      ...
      FROM hiroshi.hiroshi_LoCardDatastore_ses_buffer_storeid;")
  ...
  storeBuffer <- dbGetQuery(geoH, 
    "select * from hiroshi_LoCardData_da_store_weights")
...
if(bool_savePlotsFiles){
  ...
  customerID_regression <- readRDS("~/projects/kodyLoCardData/data_hiroshi/customerID_GLMM_smallSample.rds")
  mini_ARM <- mini %>%
